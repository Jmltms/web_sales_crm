...
const UncontactedAddLeadsBtn = ({ allLeadsRefetch }) => {
  ...
  // checking if some of the values is null or ""
  const nullValues = Object.values(input).filter(
    (value) => value === null || value === ""
  );
  // variable to disable the submit button
  const disabledBtn =
    nullValues.length !== 0 || industry.length === 0 || leadOwner === undefined;
  // handle opening of modal
  const handleOpen = () => setOpen(!open);
  // function for get the data from child modal component
  const getIndustry = (arr) => {
    setIndustry(arr);
  };
  // function to get lead owner from child component
  const getLeadOwner = (owner) => {
    setLeadOwner(owner);
  };
  // conditioning if the industry input have empty value or not
  let ind;
  if (industry.length === 0) {
    ind = <Input label="Choose from industry list" required />;
  } else {
    if (industry) {
      ind = industry?.map((name, id) => (
        <Input label="Industry" key={id} value={name ? name : ""} />
      ));
    }
  }
  ...
  const handleSave = async () => {
    const employee_id = leadOwner.split("-");
      ...
      <Dialog open={open} handler={handleOpen}>
        ...
        <DialogBody className="overflow-auto max-h-[60vh] lg:max-h-[70vh] container">
          <div className=" flex justify-between mb-2 mr-2">
            ...
            <IndustryListModal getIndustry={getIndustry} industry={industry} />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 grid-flow-row gap-2">
            <Input
              ...
              required
            />
            <Input label="Last name" onChange={handleChange} name="last_name" />
            <Input
              label="Mobile Number"
              onChange={handleChange}
              name="phone_num"
          ...
          <div className="flex justify-between mt-5 mb-2 mr-2">
            ...
            <StaffListModal getLeadOwner={getLeadOwner} />
          </div>
          <hr className=" border-blue-gray-100 mb-5" />
          <div className="grid grid-cols-1 lg:grid-cols-2 grid-flow-row gap-2">
            <Input
            ...
            <div className="relative">
              ...
              >
                ...
                <option value={parseInt("1")}>B2B</option>
                <option value={parseInt("2")}>B2C</option>
              </select>
              <label className="before:content[' '] after:content[' '] pointer-events-none absolute left-0 -top-1.5 flex h-full w-full select-none text-[11px] font-normal leading-tight text-blue-gray-400 transition-all before:pointer-events-none before:mt-[6.5px] before:mr-1 before:box-border before:block before:h-1.5 before:w-2.5 before:rounded-tl-md before:border-t before:border-l before:border-blue-gray-200 before:transition-all after:pointer-events-none after:mt-[6.5px] after:ml-1 after:box-border after:block after:h-1.5 after:w-2.5 after:flex-grow after:rounded-tr-md after:border-t after:border-r after:border-blue-gray-200 after:transition-all peer-placeholder-shown:text-sm peer-placeholder-shown:leading-[3.75] peer-placeholder-shown:text-blue-gray-500 peer-placeholder-shown:before:border-transparent peer-placeholder-shown:after:border-transparent peer-focus:text-[11px] peer-focus:leading-tight peer-focus:text-gray-900 peer-focus:before:border-t-2 peer-focus:before:border-l-2 peer-focus:before:border-gray-900 peer-focus:after:border-t-2 peer-focus:after:border-r-2 peer-focus:after:border-gray-900 peer-disabled:text-transparent peer-disabled:before:border-transparent peer-disabled:after:border-transparent peer-disabled:peer-placeholder-shown:text-blue-gray-500">
                Lead Type
              </label>
            </div>
            <Input label="Lead Source" onChange={handleChange} name="source" />
...
export default UncontactedAddLeadsBtn;
