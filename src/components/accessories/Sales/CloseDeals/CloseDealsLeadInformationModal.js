...
import CongratsEffect from "../../Reuseables/CongratsEffect";

const CloseDealsLeadInformationModal = ({ leadId, closeDealsRefetch }) => {
  const [leadInformation, setLeadInformation] = useState(false);
  const [setOpen] = useState();
  const [dropDownStatus] = useState();
  ...
  const leadStat = leadStatus.find((obj) => obj.id === detailLead?.lead_status);
      ...
      <>
        <div className=" grid grid-cols-2 gap-2 space-x-2">
          <div className="space-y-2">
            ...
            />
            <Input label="Status" value={leadStat ? leadStat.label : ""} />
            <Input
              onChange={handleChange}
              label="Mobile Number"
              name="phone_num"
        ...
        <div className=" mt-5 ">
          {detailLead.service === "" ? (
            <Input label="Service Offered" readOnly />
          ) : (
            <Input
              label="Service Offered"
              readOnly
              defaultValue={detailLead.service}
            />
          )}
        </div>
      </>
    ...
    <>
      <div>
        ...
        <div className="hidden">
          <CongratsEffect handleSave={handleSave} />
        </div>
      </div>
    </>
  );
...
export default CloseDealsLeadInformationModal;
